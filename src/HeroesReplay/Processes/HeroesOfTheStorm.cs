...
namespace HeroesReplay
{
    ...
    {
        private const int SPLIT_HOURS = 3;
        private const int SPLIT_MINUTES = 2;
        private const string TIME_SEPERATOR = ":";
        private const string STORM_INTERFACE_NAME = "AhliObs 0.66.StormInterface";
        private const string PROCESS_NAME = "HeroesOfTheStorm_x64";
        private const string LOADING_SCREEN_TEXT = "WELCOME TO";
        ...
        private const string HERO_SWITCHER_PROCESS = "HeroesSwitcher_x64.exe";

        public HeroesOfTheStorm(ILogger<HeroesOfTheStorm> logger, CancellationTokenSource source) : base(logger, PROCESS_NAME, string.Empty, source)
        {
        ...
        public async Task SetGameVariables()
        {
            ...
            values["observerinterface"] = STORM_INTERFACE_NAME;
            values["replayinterface"] = STORM_INTERFACE_NAME;
        ...
        public async Task<TimeSpan?> TryGetTimerAsync()
        {
            ...
            try
            {
                ...
                {
                    OcrResult? result = await TryGetOcrResult(centerTimer, TIME_SEPERATOR);
        ...
        private TimeSpan? TryParseTimeSpan(OcrResult ocrResult)
        {
            try
            {
                ...
                {
                    SPLIT_HOURS => TimeSpan.ParseExact(time, "hh\\:mm\\:ss", CultureInfo.InvariantCulture),

                    SPLIT_MINUTES => segments[0][0] == '-' ?
                        TimeSpan.ParseExact(time, "\\-mm\\:ss", CultureInfo.CurrentCulture, TimeSpanStyles.AssumeNegative) :
                        TimeSpan.ParseExact(time, "mm\\:ss", CultureInfo.InvariantCulture),
        ...
        private FileInfo GetSwitcherPath()
        {
            ...
            while (true)
            {
                var switcher = current.GetFiles(HERO_SWITCHER_PROCESS, SearchOption.AllDirectories).FirstOrDefault();
        ...
        public async Task<bool> WaitForMapLoadingAsync(StormReplay stormReplay, CancellationToken token = default)
        {
            ...
            return await Policy
                ...
                .ExecuteAsync(async (t) =>
                {
                    ...
                    return await GetWindowContainsAsync(WindowScreenCapture.GPU, LOADING_SCREEN_TEXT, stormReplay.Replay.Map);
