    ...
    public class CampCaptureWeightings : IGameWeightings
    {
        public IEnumerable<(Unit Unit, Player Player, double Points, string Description)> GetPlayers(TimeSpan now, Replay replay)
        {
            ...
            foreach (TrackerEvent capture in events)
            {
                ...
                foreach (Unit unit in replay.Units.Where(unit => unit.TimeSpanBorn < capture.TimeSpan && unit.TimeSpanDied < capture.TimeSpan && unit.PlayerKilledBy != null && unit.PlayerKilledBy.Team == teamId && Constants.OtherCamps.Any(oc => unit.Name.Contains(oc))))
                {
                    yield return (unit, unit.PlayerKilledBy, Constants.Weights.CampCapture, $"{unit.PlayerKilledBy.HeroId} captured {unit.Name} (CampCaptures)");
                }
            }
        }
    }
