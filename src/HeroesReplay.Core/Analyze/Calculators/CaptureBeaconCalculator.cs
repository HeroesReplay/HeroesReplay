...
namespace HeroesReplay.Core
{
    public class CaptureBeaconCalculator : IFocusCalculator
    {
        ...
        public IEnumerable<Focus> GetPlayers(TimeSpan now, Replay replay)
        {
            ...
            {
                foreach (var captureUnit in replay.Units.Where(unit => unit.TimeSpanBorn == TimeSpan.Zero && unit.TimeSpanDied == null && unit.Team == null && !settings.HeroesToolChest.IgnoreUnits.Any(u => unit.Name.Contains(u))))
                {
