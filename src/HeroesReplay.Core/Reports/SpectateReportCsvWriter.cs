...
namespace HeroesReplay.Core.Reports
{
    public class SpectateReportCsvWriter : ISpectateReportWriter
    {
        ...
        private readonly IReplayAnalzer analyzer;
        ...
        public SpectateReportCsvWriter(IGameData gameData, AppSettings settings, IReplayProvider provider, IReplayAnalzer analyzer, CancellationTokenProvider tokenProvider)
        {
        ...
        public async Task OutputReportAsync()
        {
            ...
            IEnumerable<string> lines = analyzer.GetPlayers(stormReplay.Replay)
                                                   .Select(WriteCsvLine)
