...
{
    public static TimeSpan ParseTimerHours(this string time, string format) =>
        TimeSpan.ParseExact(time, format, CultureInfo.CurrentCulture);

    public static TimeSpan ParseNegativeTimerMinutes(this string time, string format) =>
        TimeSpan.ParseExact(
            time,
            format,
