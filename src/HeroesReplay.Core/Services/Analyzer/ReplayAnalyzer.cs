...
using HeroesReplay.Core.Services.Analyzer.Calculators;
...
namespace HeroesReplay.Core.Services.Analyzer
{
    public class ReplayAnalyzer : IReplayAnalyzer
    {
        ...
        private readonly IPayloadsBuilder builder;
        ...
        {
            this.mapOptions = mapOptions.Value;
        ...
        public IReadOnlyDictionary<TimeSpan, Panel> GetPanels(Replay replay)
        {
            ...
            if (parseOptions.ShouldParseStatistics)
            {
                ...
                foreach (var talentTime in replay.TeamLevels.SelectMany(x => x).Where(x => spectateOptions.TalentLevels.Contains(x.Key)).Select(x => x.Value))
                {
        ...
        {
            return builder.CreatePayloads(replay);
        }
        ...
        public IReadOnlyDictionary<TimeSpan, Focus> GetPlayers(Replay replay)
        {
            ...
            foreach (var entry in deathEntries)
            {
                ...
                for (int second = 1; second < spectateOptions.PastDeathContextTime.TotalSeconds; second++)
                {
                ...
                for (int second = 1; second < spectateOptions.PresentDeathContextTime.TotalSeconds; second++)
                {
        ...
        public TimeSpan GetStart(Replay replay)
        {
            ...
            return replay.TrackerEvents.First(x => x.Data.dictionary[0].blobText == trackerOptions.GatesOpen).TimeSpan;
        }

        public bool GetIsCarriedObjective(Replay replay)
        {
            return replay != null && (mapOptions.CarriedObjectives.Contains(replay.Map) ||
                                      mapOptions.CarriedObjectives.Contains(replay.MapAlternativeName));
        }
