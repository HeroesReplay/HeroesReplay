...
namespace HeroesReplay.Core.Services.Analyzer.Calculators
{
    public class NearEnemyCalculator : IFocusCalculator
    {
        ...
        public NearEnemyCalculator(IOptions<WeightOptions> weightOptions, IOptions<SpectateOptions> spectateOptions, IGameData gameData)
        {
                ...
                foreach (var teamTwoUnit in teams[1])
                {
                    foreach (var teamTwoPos in teamTwoUnit.Positions.Where(p => p.TimeSpan == now))
                    {
                        foreach (var teamOnePos in teamOneUnit.Positions.Where(p => p.TimeSpan == now))
                        {
                            ...
                            if (distance <= spectateOptions.MaxDistanceToEnemy)
                            {
                                ...
                                float prioritiseCloserHero = Convert.ToSingle(distance) / weightOptions.NearEnemyHeroDistanceDivisor;

                                yield return new Focus(
                                    GetType(),
                                    target,
                                    target.PlayerControlledBy,
                                    weightOptions.NearEnemyHero + weightOptions.NearEnemyHeroOffset - prioritiseCloserHero,
