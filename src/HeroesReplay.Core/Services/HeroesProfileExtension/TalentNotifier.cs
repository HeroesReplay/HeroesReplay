...
public class TalentNotifier : ITalentNotifier
{
    ...
    {
        ExtensionPayload createReplayPayload = Data.Payloads.Create.Find(p =>
            p.Step == ExtensionStep.CreateReplayData
        );

        if (createReplayPayload != null)
        {
            string session = await extensionService
                .CreateReplaySessionAsync(createReplayPayload, token)
        ...
        {
            ExtensionPayload createPlayerPayload = Data.Payloads.Create.Find(p =>
                p.Step == ExtensionStep.CreatePlayerData
            );

            if (createPlayerPayload != null)
            {
                bool success = await extensionService
                    .CreatePlayerDataAsync(createPlayerPayload, SessionId, token)
                    .ConfigureAwait(false);
    ...
    {
        ExtensionPayload updateReplayPayload = Data.Payloads.Update.Find(p =>
            p.Step == ExtensionStep.UpdateReplayData
        );

        if (updateReplayPayload != null)
        {
            bool success = await extensionService
                .UpdateReplayDataAsync(updateReplayPayload, SessionId, token)
                .ConfigureAwait(false);
        ...
        {
            ExtensionPayload updatePlayerPayload = Data.Payloads.Update.Find(p =>
                p.Step == ExtensionStep.UpdatePlayerData
            );

            if (updatePlayerPayload != null)
            {
                bool success = await extensionService
                    .UpdatePlayerDataAsync(updatePlayerPayload, SessionId, token)
                    .ConfigureAwait(false);
    ...
    private async Task UpdateTalentsAsync(TimeSpan timer, CancellationToken token = default)
    {
        foreach (TimeSpan talentTime in Data.Payloads.Talents.Keys)
        {
            if (talentTime <= timer)
            {
                ...
                bool success = await extensionService
                    .UpdatePlayerTalentsAsync(Data.Payloads.Talents[talentTime], SessionId, token)
                    .ConfigureAwait(false);
                ...
                if (success && Data.Payloads.Talents.Remove(talentTime))
                {
                    ...
                    bool notified = await extensionService
                        .NotifyTwitchAsync(token)
                        .ConfigureAwait(false);
