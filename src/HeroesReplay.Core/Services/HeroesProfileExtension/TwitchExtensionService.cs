...
public class TwitchExtensionService : ITwitchExtensionService
{
    ...
    public async Task<string> CreateReplaySessionAsync(
    ...
    )
    {
        ...
        try
        {
            using (
                CancellationTokenSource cancellationSource =
                    CancellationTokenSource.CreateLinkedTokenSource(token, tokenProvider.Token)
            )
            {
                    ...
                    .ExecuteAsync(
                        async (context, token) =>
                            await httpClient.PostAsync(
                                SaveReplayUrl,
                                new FormUrlEncodedContent(payload.Content.Single()),
        ...
        {
            logger.LogWarning(
                "Could not create twitch extension replay session because the task was cancelled."
            );
        }
        catch (Exception e)
        {
    ...
    public async Task<bool> CreatePlayerDataAsync(
        ExtensionPayload payload,
        string sessionId,
        CancellationToken token
    )
    {
        ...
        {
            logger.LogWarning(
                "Could not create twitch extension player data because the task was cancelled."
            );
        }
        catch (Exception e)
        {
    ...
    public async Task<bool> UpdateReplayDataAsync(
        ExtensionPayload payload,
        string sessionId,
        CancellationToken token
    )
    {
        ...
        {
            logger.LogWarning(
                "Could not update twitch extension replay data because the task was cancelled."
            );
        }
        catch (Exception e)
        {
    ...
    public async Task<bool> UpdatePlayerDataAsync(
        ExtensionPayload payload,
        string sessionId,
        CancellationToken token
    )
    {
                    ...
                    HttpResponseMessage response = await Policy
                        ...
                        .ExecuteAsync(
                            async (context, t) =>
                                await httpClient.PostAsync(
                                    UpdatePlayerUrl,
                                    new FormUrlEncodedContent(playerContent),
                                    t
                                ),
                            new PollyContext(),
                            cancellationSource.Token
        ...
        {
            logger.LogWarning(
                "Could not update twitch extension player data because the task was cancelled."
            );
        }
        catch (Exception e)
        {
    ...
    public async Task<bool> UpdatePlayerTalentsAsync(
        List<ExtensionPayload> talentPayloads,
        string sessionId,
        CancellationToken token
    )
    {
        ...
        {
            logger.LogWarning(
                "Could not update twitch extension talents because the task was cancelled."
            );
        }
        catch (Exception e)
        {
        ...
        try
        {
            ...
            )
            {
                HttpResponseMessage response = await Policy
                    ...
                    .ExecuteAsync(
                        async (context, token) =>
                            await httpClient.PostAsync(NotifyTalentUpdate, notifyContent, token),
                        new PollyContext(),
                        cancellationSource.Token
                    )
                    .ConfigureAwait(false);
