...
public class YouTubeUploader : IYouTubeUploader
{
    ...
    public async Task ListenAsync()
    {
        ...
        {
            using (
                var recordingWatcher = new FileSystemWatcher(settings.ContextsDirectory, "*.mp4")
                {
    ...
    public async Task ProcessRecording(string recordingPath)
    {
        try
        {
            ...
            var entryFile = recording
                .Directory.GetFiles("*.json", SearchOption.TopDirectoryOnly)
                .FirstOrDefault(file =>
                    file.Name.Equals(
            ...
            if (entryFile != null && entryFile.Exists)
            {
                YouTubeEntry entry = JsonSerializer.Deserialize<YouTubeEntry>(
                    await File.ReadAllTextAsync(entryFile.FullName)
                );
                ...
                using (
                    var stream = new FileStream(
                        Path.Combine(settings.Location.DataDirectory, "client_secrets.json"),
                        FileMode.Open,
                        FileAccess.Read
                    )
                )
