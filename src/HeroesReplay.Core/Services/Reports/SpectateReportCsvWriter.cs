...
public class SpectateReportCsvWriter : ISpectateReportWriter
{
    ...
    public SpectateReportCsvWriter(
        IGameData gameData,
    ...
    public async Task OutputReportAsync()
    {
        ...
        IEnumerable<string> lines = analyzer
            .GetPlayers(loadedReplay.Replay)
            .Select(WriteCsvLine)
        ...
        string report = Path.Combine(
            settings.SpectateReportPath,
            Path.GetFileName(loadedReplay.FileInfo.FullName) + ".csv"
        );
