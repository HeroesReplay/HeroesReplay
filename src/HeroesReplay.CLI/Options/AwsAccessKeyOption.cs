...
namespace HeroesReplay.CLI.Options
{
    public class AwsAccessKeyOption : Option
    {
        ...
        {
            Required = string.IsNullOrWhiteSpace(Environment.GetEnvironmentVariable("HEROES_REPLAY_AWS_ACCESS_KEY"));
            Argument = new Argument<string>(() => Environment.GetEnvironmentVariable("HEROES_REPLAY_AWS_ACCESS_KEY", EnvironmentVariableTarget.Process));
        }
    }
}