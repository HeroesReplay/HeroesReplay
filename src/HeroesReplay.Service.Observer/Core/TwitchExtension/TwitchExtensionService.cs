...
namespace HeroesReplay.Service.Spectator.Core.HeroesProfileExtension
{
    public class TwitchExtensionService : ITwitchExtensionService
    {
            ...
            try
            {
                using (CancellationTokenSource cancellationSource = CancellationTokenSource.CreateLinkedTokenSource(token, tokenProvider.Token))
                {
                    HttpResponseMessage response = await Policy
                               ...
                               .ExecuteAsync(async (context, token) => await httpClient.PostAsync(SaveReplayUrl, new FormUrlEncodedContent(payload.Content.Single()), cancellationSource.Token), new Polly.Context(), tokenProvider.Token)
                ...
                using (CancellationTokenSource cancellationSource = CancellationTokenSource.CreateLinkedTokenSource(token, tokenProvider.Token))
                {
                    foreach (var content in payload.Content)
                    {
                        HttpResponseMessage response = await Policy
                            ...
                            .ExecuteAsync(async (context, token) => await httpClient.PostAsync(SavePlayerUrl, new FormUrlEncodedContent(content), token), new Polly.Context(), cancellationSource.Token)
                            .ConfigureAwait(false);
            ...
            try
            {
                ...
                using (CancellationTokenSource cancellationSource = CancellationTokenSource.CreateLinkedTokenSource(token, tokenProvider.Token))
                {
                    HttpResponseMessage response = await Policy
                               ...
                               .OrResult<HttpResponseMessage>(msg => !msg.IsSuccessStatusCode)
                               .WaitAndRetryAsync(retryCount: 5, sleepDurationProvider: GetSleepDuration, onRetry: OnRetry)
                               .ExecuteAsync(async (context, token) => await httpClient.PostAsync(UpdateReplayUrl, new FormUrlEncodedContent(payload.Content.Single()), token), new Polly.Context(), cancellationSource.Token)
                               .ConfigureAwait(false);
                ...
                using (CancellationTokenSource cancellationSource = CancellationTokenSource.CreateLinkedTokenSource(token, tokenProvider.Token))
                {
                    foreach (var playerContent in payload.Content)
                    {
                        HttpResponseMessage response = await Policy
                                 ...
                                 .OrResult<HttpResponseMessage>(msg => !msg.IsSuccessStatusCode)
                                 .WaitAndRetryAsync(retryCount: 5, sleepDurationProvider: GetSleepDuration, onRetry: OnRetry)
                                 .ExecuteAsync(async (context, t) => await httpClient.PostAsync(UpdatePlayerUrl, new FormUrlEncodedContent(playerContent), t), new Polly.Context(), cancellationSource.Token)
                                 .ConfigureAwait(false);
                    ...
                    foreach (var talentPayload in talentPayloads)
                    {
                        ...
                        foreach (var content in talentPayload.Content)
                        {
                            HttpResponseMessage response = await Policy
                                ...
                                .OrResult<HttpResponseMessage>(msg => !msg.IsSuccessStatusCode)
                                .WaitAndRetryAsync(retryCount: 10, sleepDurationProvider: GetSleepDuration, onRetry: OnRetry)
                                .ExecuteAsync(async (context, token) => await httpClient.PostAsync(SaveTalentUrl, new FormUrlEncodedContent(content), token), new Polly.Context(), cancellationSource.Token)
                                .ConfigureAwait(false);
            ...
            try
            {
                using (CancellationTokenSource cancellationSource = CancellationTokenSource.CreateLinkedTokenSource(token, tokenProvider.Token))
                {
                    HttpResponseMessage response = await Policy
                                     ...
                                     .OrResult<HttpResponseMessage>(msg => !msg.IsSuccessStatusCode)
                                     .WaitAndRetryAsync(retryCount: 5, sleepDurationProvider: GetSleepDuration, onRetry: OnRetry)
                                     .ExecuteAsync(async (context, token) => await httpClient.PostAsync(NotifyTalentUpdate, notifyContent, token), new Polly.Context(), cancellationSource.Token)
                                     .ConfigureAwait(false);
        ...
        private TimeSpan GetSleepDuration(int retry, Polly.Context context)
        {
        ...
        private void OnRetry(DelegateResult<HttpResponseMessage> wrappedResponse, TimeSpan timeSpan, int retryAttempt, Polly.Context context)
        {
